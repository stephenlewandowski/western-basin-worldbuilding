...
ACTORS = [
    ...
    ["GA-021", "Great Lakes Indian Fish & Wildlife Commission", "intertribal_body", "tribal", "Treaty-ceded territories served by member Ojibwe tribes", "Ecology / Biodiversity;Water;Data / Sensors / Security", "Intertribal natural-resource management, treaty-rights support, conservation enforcement support, legal/policy analysis, and public information.", "s17_glifwc", "high", "real", "verified", "GLIFWC is an intertribal resource body, not a single sovereign nation; it is not assigned current Western Basin jurisdiction without nation- and treaty-specific evidence."],
...
SOURCES = [
    ...
    ["s47_epa_great_lakes_monitoring", "Great Lakes Monitoring", "https://www.epa.gov/great-lakes-monitoring", "federal_scientific_program", "Great Lakes monitoring and assessment", "current page", "2026-09-04", "Great Lakes", "Monitoring data support assessment and decisions; they do not by themselves confer local operating or regulatory authority.", "retrieved"],
]
...
add_auth(4,"GA-001","Water","monitor","monitoring","not_applicable","EPA Great Lakes monitoring and assessment program","Great Lakes","federal","s47_epa_great_lakes_monitoring","moderate","verified","Monitoring and assessment provide information; they do not operate local utilities or regulate every source.")
...
add_auth(30,"GA-009","Ecology / Biodiversity","provide_data","scientific_information","not_applicable","National Wetlands Inventory mapping and status/trends information","United States","federal","s08_usfws_nwi","high","verified","NWI data support decisions but do not themselves confer restoration, permit, or enforcement authority.")
...
def render_map(map_number: int) -> Path:
    ...
    svg_text = re.sub(r'id="p[0-9a-f]+"', 'id="p5960d182a0"', svg_text)
    svg_text = re.sub(r'url\(#p[0-9a-f]+\)', 'url(#p5960d182a0)', svg_text)
