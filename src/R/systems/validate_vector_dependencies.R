...
a_hash_hit <- regexec('"protected_12a_manifest"[[:space:]]*:[[:space:]]*\\{.*?"sha256"[[:space:]]*:[[:space:]]*"([0-9a-f]{64})".*?"bytes"[[:space:]]*:[[:space:]]*([0-9]+)', b_txt, perl=TRUE)
a_hash <- regmatches(b_txt, a_hash_hit)[[1]]
stop_if(length(a_hash)==3L, "Phase 12A protected-manifest metadata")
stop_if(portable_match(A_MANIFEST, "reports/vector_ecology_baseline_manifest.json", a_hash[[2]]), "Phase 12A working-baseline hash")
a_raw <- raw_bytes(A_MANIFEST); a_canonical <- canonical_bytes(a_raw); a_crlf <- charToRaw(gsub("\n", "\r\n", rawToChar(a_canonical), fixed=TRUE))
stop_if(as.numeric(a_hash[[3]]) %in% c(length(a_raw), length(a_canonical), length(a_crlf)), "Phase 12A working-baseline bytes")
